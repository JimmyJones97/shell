...
void
dvmInterpret(JNIEnv *env, jobject instance, const Method *curMethod, u4 *reg, jvalue *pResult) {

    /* core state */
    const u2 *pc;               // program counter
    const u2 *curInsns = curMethod->insns;
...
void
dvmCallMethod(JNIEnv *env, jobject instance, const Method *method, jvalue *pResult, ...) {
    // 参数入寄存器
    int verifyCount = 0;
...
InstField *(*sysDvmFindInstanceFieldHider)(const ClassObject *clazz, const char *fieldName,
                                           const char *signature) = nullptr;


InstField *
fakeDvmFindInstanceFieldHider(const ClassObject *clazz, const char *fieldName,
                              const char *signature) {
    if (fakeGetFieldID && strcmp("00", signature) == 0) {
        fakeGetFieldID = false;
        return (InstField *) clazz;
    }
    return sysDvmFindInstanceFieldHider(clazz, fieldName, signature);
}

void hookDvmFindInstanceFieldHider() {
    static bool flag = false;
    if (!flag) {
        HookNativeInline("/system/lib/libdvm.so",
                         "_Z24dvmFindInstanceFieldHierPK11ClassObjectPKcS3_",
                         reinterpret_cast<void *>(fakeDvmFindInstanceFieldHider),
                         reinterpret_cast<void **>(&sysDvmFindInstanceFieldHider));
        flag = true;
    }
}
...
//    static bool flag = false;
//    if (!flag) {
//        HookNativeAnonymityInline("/system/lib/libdvm.so",
...
int dvmFindCatchBlock(const Method *method, size_t pcOff, jobject exception) {
    ...
    const CodeItemData *pCodeItemData = getCodeItem(method);
    const DexTry *pDexTry = reinterpret_cast<const DexTry *>(pCodeItemData->triesAndHandlersBuf);
...
void initMethodByCodeItem(Method *method) {
    const CodeItemData *pCode = getCodeItem(method);
    method->registersSize = pCode->registersSize;
    method->insSize = pCode->insSize;
    method->outsSize = pCode->outSize;
    method->insns = pCode->insns;
}

const CodeItemData *getCodeItem(const Method *method) {
    // (sign + method_name + class_name).reverse
    string key;
    dvmResolveMethodSign(method->clazz, &method->prototype, &key);
    key += method->name;
    key += method->clazz->descriptor;
    reverse(key.begin(), key.end());
    return getCodeItemByKey(key);
}
