/*
   minizip.c v1.15 - public domain deflate/inflate, zlib-subset, ZIP
   ...
   Most API's defined in minizip.c are optional. For example, to disable the
   archive related functions just define MINIZIP_NO_ARCHIVE_APIS, or to get rid of
   all stdio usage define MINIZIP_NO_STDIO (see the list below for more macros).
   ...
   * Change History
   ...
   re-ran a randomized regression test on ~500k files.
       ...
       - Ran all examples, minizip.c, and tinfl.c through MSVC 2008's /analyze
   ...
   Linux+Win32/x64.
       - Added minizip_tester solution/project, which is a useful little app
   derived from LZHAM's tester app that I use as part of the regression test.
       - Ran minizip.c and tinfl.c through another series of regression testing on
   ~500,000 files and archives.
       - Modified example5.c so it purposely disables a bunch of high-level
   functionality (MINIZIP_NO_STDIO, etc.). (Thanks to corysama for the
   MINIZIP_NO_STDIO bug report.)
       - Fix ftell() usage in examples so they exit with an error on files which
   are too large (a limitation of the examples, not minizip itself). 4/12/12 v1.12
   ...
   * zlib-style API notes:
     ...
     minizip.c implements a fairly large subset of zlib. There's enough
   ...
   guarantees that minizip.c pulls this off perfectly.
   ...
   * This is a header file library, like stb_image.c. To get only a header file,
   either cut and paste the below header, or create minizip.h, #define
   MINIZIP_HEADER_FILE_ONLY, and then include minizip.c from it.

   * Important: For best perf. be sure to customize the below macros for your
   target platform: #define MINIZIP_USE_UNALIGNED_LOADS_AND_STORES 1 #define
   MINIZIP_LITTLE_ENDIAN 1 #define MINIZIP_HAS_64BIT_REGISTERS 1

   * On platforms using glibc, Be sure to "#define _LARGEFILE64_SOURCE 1" before
   including minizip.c to ensure minizip uses the 64-bit variants: fopen64(),
   stat64(), etc. Otherwise you won't be able to process large files (i.e.
   32-bit stat() fails for me on files > 0x7FFFFFFF bytes).
*/

#ifndef MINIZIP_HEADER_INCLUDED
#define MINIZIP_HEADER_INCLUDED
...
// Define MINIZIPIP_NO_STDIO to disable all usage and any functions which rely on
// stdio for file I/O.
//#define MINIZIPIP_NO_STDIO

// If MINIZIPIP_NO_TIME is specified then the ZIP archive functions will not be able
...
//#define MINIZIPIP_NO_TIME

// Define MINIZIPIP_NO_ARCHIVE_APIS to disable all ZIP archive API's.
//#define MINIZIPIP_NO_ARCHIVE_APIS

// Define MINIZIPIP_NO_ARCHIVE_APIS to disable all writing related ZIP archive
// API's.
//#define MINIZIPIP_NO_ARCHIVE_WRITING_APIS

// Define MINIZIPIP_NO_ZLIB_APIS to remove all ZLIB-style compression/decompression
// API's.
//#define MINIZIPIP_NO_ZLIB_APIS

// Define MINIZIPIP_NO_ZLIB_COMPATIBLE_NAME to disable zlib names, to prevent
// conflicts against stock zlib.
//#define MINIZIPIP_NO_ZLIB_COMPATIBLE_NAMES

// Define MINIZIPIP_NO_MALLOC to disable all calls to malloc, free, and realloc.
// Note if MINIZIPIP_NO_MALLOC is defined then the user must always provide custom
...
//#define MINIZIPIP_NO_MALLOC

#if defined(__TINYC__) && (defined(__linux) || defined(__linux__))
// TODO: Work around "error: include file 'sys\utime.h' when compiling with tcc
// on Linux
#define MINIZIP_NO_TIME
#endif

#if !defined(MINIZIP_NO_TIME) && !defined(MINIZIP_NO_ARCHIVE_APIS)
...
// MINIZIP_X86_OR_X64_CPU is only used to help set the below macros.
#define MINIZIP_X86_OR_X64_CPU 1
#endif

#if (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__) || MINIZIP_X86_OR_X64_CPU
// Set MINIZIP_LITTLE_ENDIAN to 1 if the processor is little endian.
#define MINIZIP_LITTLE_ENDIAN 1
#endif

/* Set MINIZIP_USE_UNALIGNED_LOADS_AND_STORES only if not set */
#if !defined(MINIZIP_USE_UNALIGNED_LOADS_AND_STORES)
#if MINIZIP_X86_OR_X64_CPU
/* Set MINIZIP_USE_UNALIGNED_LOADS_AND_STORES to 1 on CPU's that permit efficient integer loads and stores from unaligned addresses. */
#define MINIZIP_USE_UNALIGNED_LOADS_AND_STORES 1
#define MINIZIP_UNALIGNED_USE_MEMCPY
#else
#define MINIZIP_USE_UNALIGNED_LOADS_AND_STORES 0
#endif
#endif
    ...
    defined(__x86_64__)
// Set MINIZIP_HAS_64BIT_REGISTERS to 1 if operations on 64-bit integers are
// reasonably fast (and don't involve compiler generated calls to helper
// functions).
#define MINIZIP_HAS_64BIT_REGISTERS 1
#endif
...
// For more compatibility with zlib, minizip.c uses unsigned long for some
...
void *def_realloc_func(void *opaque, void *address, size_t items,
                       size_t size);

#if MINIZIP_USE_UNALIGNED_LOADS_AND_STORES && MINIZIP_LITTLE_ENDIAN
...
#define MZ_VER_NUM 0x91F0
#define MZ_VER_MAJOR 9
#define MZ_VER_MINOR 1
#define MZ_VER_REVISION 15
#define MZ_VER_SUB_REVISION 0
...
enum {
    ...
    MZ_USUAL_BETTER_COMPRESSION = 10,
    MZ_DEFAULT_LEVEL = 6,
...
//  currently only enabled when MINIZIP_USE_UNALIGNED_LOADS_AND_STORES and
//  MINIZIP_LITTLE_ENDIAN are defined.)
// Return values:
//  MZ_OK on success.
//  MZ_STREAM_ERROR if the stream is bogus.
//  MZ_PARAM_ERROR if the input parameters are bogus.
//  MZ_MEM_ERROR on out of memory.
int mz_deflateInit(mz_stream_p pStream, int level);
...
//   ignored by minizip.c)
int mz_deflateInit2(mz_stream_p pStream, int level, int method, int window_bits,
...
int mz_deflateReset(mz_stream_p pStream);
...
//   space and try again.)
int mz_deflate(mz_stream_p pStream, int flush);

// mz_deflateEnd() de-initializes a compressor:
// Return values:
//  MZ_OK on success.
//  MZ_STREAM_ERROR if the stream is bogus.
int mz_deflateEnd(mz_stream_p pStream);
...
mz_ulong mz_deflateBound(mz_stream_p pStream, mz_ulong source_len);
...
int mz_inflateInit(mz_stream_p pStream);
...
int mz_inflateInit2(mz_stream_p pStream, int window_bits);
...
int mz_inflate(mz_stream_p pStream, int flush);

// De-initializes a decompressor.
int mz_inflateEnd(mz_stream_p pStream);
...
// Redefine zlib-compatible names to minizip equivalents, so minizip.c can be used
// as a drop-in replacement for the subset of zlib that minizip.c supports. Define
// MINIZIP_NO_ZLIB_COMPATIBLE_NAMES to disable zlib-compatibility if you use zlib
// in the same project.
#ifndef MINIZIP_NO_ZLIB_COMPATIBLE_NAMES
...
#define ZLIB_VERNUM MZ_VER_NUM
#define ZLIB_VER_MAJOR MZ_VER_MAJOR
#define ZLIB_VER_MINOR MZ_VER_MINOR
#define ZLIB_VER_REVISION MZ_VER_REVISION
#define ZLIB_VER_SUBREVISION MZ_VER_SUB_REVISION
#define zlibVersion mz_version
#define zlib_version mz_version()
#endif // #ifndef MINIZIP_NO_ZLIB_COMPATIBLE_NAMES

#endif // MINIZIP_NO_ZLIB_APIS
...
#ifndef MINIZIP_NO_ARCHIVE_APIS

#ifdef MINIZIP_NO_STDIO
#define MZ_FILE void *
#else
...
typedef struct {
    ...
    char m_file_name[MZ_ZIP_MAX_ARCHIVE_FILENAME_SIZE];
...
// alignment and platform endian issues, minizip.c doesn't use structs for any of
// this stuff.
enum {
    ...
    MZ_ZIP_END_CDH_SIG_OFS = 0,
    MZ_ZIP_END_CDH_NUM_THIS_DISK_OFS = 4,
    MZ_ZIP_END_CDH_NUM_DISK_CENTRAL_DIR_OFS = 6,
    MZ_ZIP_END_CDH_CENTRAL_DIR_NUM_ENTRIES_ON_DISK_OFS = 8,
    MZ_ZIP_END_CDH_CENTRAL_DIR_TOTAL_ENTRIES_OFS = 10,
    MZ_ZIP_END_CDH_CENTRAL_DIR_SIZE_OFS = 12,
    MZ_ZIP_END_CDH_CENTRAL_DIR_OFS_OFS = 16,
    MZ_ZIP_END_CDH_COMMENT_SIZE_OFS = 20,
};
...
#define MZ_TO_LOWER(c) ((((c) >= 'A') && ((c) <= 'Z')) ? ((c) - 'A' + 'a') : (c))
...
MZ_FORCEINLINE void mz_zip_array_clear(mz_zip_archive *pZip,
                                       mz_zip_array *pArray);

mz_bool mz_zip_array_ensure_capacity(mz_zip_archive *pZip,
                                     mz_zip_array *pArray,
                                     size_t min_new_capacity,
                                     mz_uint growing);

MZ_FORCEINLINE mz_bool mz_zip_array_reserve(mz_zip_archive *pZip,
                                            mz_zip_array *pArray,
                                            size_t new_capacity,
                                            mz_uint growing);

MZ_FORCEINLINE mz_bool mz_zip_array_resize(mz_zip_archive *pZip,
                                           mz_zip_array *pArray,
                                           size_t new_size,
                                           mz_uint growing);

MZ_FORCEINLINE mz_bool mz_zip_array_ensure_room(mz_zip_archive *pZip,
                                                mz_zip_array *pArray,
                                                size_t n);

MZ_FORCEINLINE mz_bool mz_zip_array_push_back(mz_zip_archive *pZip,
                                              mz_zip_array *pArray,
...
void mz_zip_time_t_to_dos_time(time_t time, mz_uint16 *pDOS_time,
                               mz_uint16 *pDOS_date);

mz_bool mz_zip_get_file_modified_time(const char *pFilename,
                                      time_t *pTime);

mz_bool mz_zip_set_file_times(const char *pFilename, time_t access_time,
                              time_t modified_time);

mz_bool mz_zip_reader_init_internal(mz_zip_archive *pZip,
                                    mz_uint32 flags);

MZ_FORCEINLINE mz_bool
...
mz_bool mz_zip_reader_read_central_dir(mz_zip_archive *pZip,
                                       mz_uint32 flags);
...
size_t mz_zip_mem_read_func(void *pOpaque, mz_uint64 file_ofs,
                            void *pBuf, size_t n);

mz_bool mz_zip_reader_init_mem(mz_zip_archive *pZip, const void *pMem,
                               size_t size, mz_uint32 flags);

#ifndef MINIZIP_NO_STDIO

size_t mz_zip_file_read_func(void *pOpaque, mz_uint64 file_ofs,
                             void *pBuf, size_t n);
...
MZ_FORCEINLINE mz_bool mz_zip_reader_string_equal(const char *pA,
                                                  const char *pB,
                                                  mz_uint len,
                                                  mz_uint flags);
...
int mz_zip_reader_locate_file_binary_search(mz_zip_archive *pZip,
...
#ifndef MINIZIP_NO_ARCHIVE_WRITING_APIS

// Inits a ZIP archive writer.
mz_bool mz_zip_writer_init(mz_zip_archive *pZip, mz_uint64 existing_size);

size_t mz_zip_heap_write_func(void *pOpaque, mz_uint64 file_ofs,
                              const void *pBuf, size_t n);
...
size_t mz_zip_file_write_func(void *pOpaque, mz_uint64 file_ofs,
                              const void *pBuf, size_t n);
...
mz_bool mz_zip_writer_add_put_buf_callback(const void *pBuf, int len,
                                           void *pUser);
...
mz_bool mz_zip_writer_validate_archive_name(const char *pArchive_name);

mz_uint
mz_zip_writer_compute_padding_needed_for_file_alignment(mz_zip_archive *pZip);

mz_bool mz_zip_writer_write_zeros(mz_zip_archive *pZip,
...
// re-compression), along with its full filename, extra data, and comment fields.
...
#endif // #ifndef MINIZIP_NO_ARCHIVE_WRITING_APIS

#endif // #ifndef MINIZIP_NO_ARCHIVE_APIS
...
//  than src_buf_len on un-compressible data. The caller must call mz_free() on
...
tinfl_status tinfl_decompress(tinfl_decompressor *r,
                              ...
                              const mz_uint32 decompress_flags);
...
#if MINIZIP_HAS_64BIT_REGISTERS
#define TINFL_USE_64BIT_BITBUF 1
#endif

#if TINFL_USE_64BIT_BITBUF
typedef mz_uint64 tinfl_bit_buf_t;
#define TINFL_BIT_BUF_SIZE (64)
#else
typedef mz_uint32 tinfl_bit_buf_t;
#define TINFL_BIT_BUF_SIZE (32)
#endif
...
mz_bool tdefl_output_buffer_putter(const void *pBuf, int len,
                                   void *pUser);
...
// On entry:
//  pImage, w, h, and num_channels describe the image to compress. num_channels may be
//  1, 2, 3, or 4. The image pitch in bytes per scan_line will be w*num_channels.
//  The leftmost pixel on the top scan_line is stored first in memory. level may
...
// There is no corresponding de-init() function because the tdefl API's do not
...
tdefl_sym_freq *tdefl_radix_sort_syms(mz_uint num_syms,
...
void tdefl_huffman_enforce_max_code_size(int *pNum_codes,
                                         int code_list_len,
                                         int max_code_size);

void tdefl_optimize_huffman_table(tdefl_compressor *d, int table_num,
...
MZ_FORCEINLINE void tdefl_record_literal(tdefl_compressor *d,
                                         mz_uint8 lit);
...
// Can't use tdefl_create_comp_flags_from_zip_params if MINIZIP_NO_ZLIB_APIS isn't
...
#endif // #ifndef MINIZIP_NO_ZLIB_APIS

#ifdef __cplusplus
}
#endif

#endif // MINIZIP_HEADER_INCLUDED

// ------------------- End of Header: Implementation follows. (If you only want
// the header, define MINIZIP_HEADER_FILE_ONLY.)
