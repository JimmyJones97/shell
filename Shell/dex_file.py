...
class TryItem(ABCItem):
    def parse(self, buf: bytes, offset: Index, **kwargs):
        self.start_addr, self.insn_count, self.handler_off = unpack_from('<I2H', buf, offset.index)
        offset.index += 0x08
        return self
    ...
    def to_bytes(self, offset: Index = None) -> bytes:
        return pack('<I2H', self.start_addr, self.insn_count, self.handler_off)
...
class DexHeader:
    ...
    def update_header(self, buf: bytearray, map_list: MapList):
        ...
        buf[0x08:0x0c] = pack('<I', adler32(buf[0x08:]))
...
class JNIFuncBuilder:
            ...
            if not filter_flag:
                ...
                for method in class_data.direct_methods:
                    ...
                    if method.code and method_name != r'<clinit>' and method_name != r'<init>':
                        copy_method = class_name, JNIFuncBuilder.copy_and_native(method, cur_method_idx)
    ...
    def write(self, code_item_out_path: str,
              dex_out_path: str,
    ...
    def generate_jni_func(self, jni_func_header_name: str):
        ...
        self.jni_func_header_buf = r"""
            ...
            #ifndef SHELLAPP_JNIFUNC_H
            #define SHELLAPP_JNIFUNC_H
        ...
        for class_name, method in self.methods:
            ...
            self.jni_func_buf += """
                ...
                Method methodImg = *(Method *) m_{method_name};
    ...
    def set_code_item_zero(self, dex_out_path: str):
        with open(dex_out_path, 'rb+') as file:
            dex_buf = bytearray(file.read())
            ...
            dex_buf[0x08:0x0c] = pack('<I', adler32(dex_buf[0x08:]))
    ...
    @staticmethod
    def copy_and_native(method: EncodedMethod, cur_method_idx: int) -> EncodedMethod:
