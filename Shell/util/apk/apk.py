...
class APK:
    ...
    @classmethod
    def gradle_build_debug_apk(cls, cwd, log=True):
        cls.log.debug('gradle_build_debug_apk is running...')
        child = Popen(
            [cwd + r'\gradlew.bat', 'assembleDebug', '--info', '--stacktrace'],
    ...
    def __log(cls, child, log):
        ...
        if log:
            if out_str:
                cls.log.debug(str(out_str, encoding=chardet.detect(out_str)['encoding']))
            if err_str:
                cls.log.error(str(err_str, encoding=chardet.detect(err_str)['encoding']))
