...
class APK:
    ...
    @staticmethod
    def gradle_build_debug_apk(gradle_win_bat_path, log=True):
        child = Popen(
            [gradle_win_bat_path, 'clean', 'build', '--refresh-dependencies', '--info', '--debug', '--stacktrace'],
            stdout=PIPE, stderr=PIPE)
        APK.__log(child, log)

        child = Popen(
            [gradle_win_bat_path, 'assembleDebug'],
            stdout=PIPE, stderr=PIPE)
        APK.__log(child, log)
